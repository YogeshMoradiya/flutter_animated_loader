...
class HexaSpin extends StatefulWidget {
  final double size;
  final Color color;

  const HexaSpin({super.key, required this.size, required this.color});

  @override
  BuildSpinnerState createState() => BuildSpinnerState();
}

class BuildSpinnerState extends State<HexaSpin>
          ...
          child: _animationController.value <= 0.28
              ? Stack(
                  ...
                  children: <Widget>[
                    ...
                    _buildInitialDot(
                      angle60 + angle30,
                      const Interval(0.04, 0.12),
                    ),
                    _buildInitialDot(
                      angle120 + angle30,
                      const Interval(0.08, 0.16),
                    ),
                    _buildInitialDot(
                      angle180 + angle30,
                      const Interval(0.12, 0.20),
                    ),
                    _buildInitialDot(
                      angle240 + angle30,
                      const Interval(0.16, 0.24),
                    ),
                    _buildInitialDot(
                      angle300 + angle30,
                      const Interval(0.20, 0.28),
                    ),
                  ],
                )
              : Transform.rotate(
                  ...
                  child: Stack(
                    ...
                    children: <Widget>[
                      ...
                      _buildLaterDot(
                        angle60 + angle30,
                        const Interval(0.76, 0.84),
                      ),
                      _buildLaterDot(
                        angle120 + angle30,
                        const Interval(0.72, 0.80),
                      ),
                      _buildLaterDot(
                        angle180 + angle30,
                        const Interval(0.68, 0.76),
                      ),
                      _buildLaterDot(
                        angle240 + angle30,
                        const Interval(0.64, 0.72),
                      ),
                      _buildLaterDot(
                        angle300 + angle30,
                        const Interval(0.60, 0.68),
                      ),
                    ],
                  ),
                ),
